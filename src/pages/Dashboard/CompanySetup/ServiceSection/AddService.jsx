...
const AddService = () => {
    ...
    const handleFormSubmit = (e) => {
        ...
        // Send to the server;
        api.post('/add-service', serviceInfo)
            .then(res => {
                if (res.status == 'ok') {
    ...
    const handleUpdateFormSubmit = (e) => {
        ...
        // Send to the server;
        api.put('/update-service', serviceInfo)
            .then(res => {
                if (res.status == 'ok') {
    ...
    const handleDelete = () => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                // delete api
                api.delete(`/delete-service?id=${id}`)
                    .then(res => {
                        if (res.status == 'ok') {
    ...
    return (
        <div className='min-h-screen dashbg'>
            <PageHeading title={`Company Services`} />
            <div>
                                    ...
                                    <>
                                        ...
                                        <div className='flex justify-center  py-4'>
                                            <form onSubmit={handleFormSubmit}>
                                                <input type="text" name='title' className='input input-bordered mr-2' placeholder='enter service name' required />
                                                        ...
                                                        {/* row 1 */}
                                                        {
                                                            services.map((serv, index) => <React.Fragment key={index}>
                                                                <tr className={`${(index + 1) % 2 == 0 ? 'bg-slate-200' : ''}`}>
                                                                    ...
                                                                    <td>{serv?.title}</td>
                                                                    <td>
                                                                        <Link onClick={() => handleUpdate(serv?.id)}><button className='mr-2' onClick={() => document.getElementById('my_modal_7').showModal()}>  <i className="fa-solid fa-pen-to-square"></i>  </button></Link>
                    ...
                    <div className="modal-box">
                        ...
                        <div className='py-4'>
                            <form onSubmit={handleUpdateFormSubmit}>
                                <input type="text" name='title' className='input input-bordered mr-2 mb-2' placeholder='enter service name' required />
